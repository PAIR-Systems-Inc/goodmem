...
/**
 * Represents an API Key, using immutable ByteString for key material.
 * Encapsulates its raw material, hashed version for storage,
 * and user-facing string representation using Base32 encoding.
 *
 * Use the static factory method `newKey` to generate new keys.
 */
public record ApiKey(
     ...
     * @param numBytes The desired number of random bytes for the key material (e.g., 24 for ~192 bits). Must be positive.
    ...
    public static ApiKey newKey(SecureRandom secureRandom, int numBytes) {
        ...
        byte[] hashedMaterialBytes;
        try {
            MessageDigest digest = MessageDigest.getInstance(HASH_ALGORITHM);
            hashedMaterialBytes = digest.digest(rawMaterialBytes);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  Length of random part (Base32): " + (key1.keyString().length() - KEY_PREFIX.length()));
        System.out.println("  Key Material (Hex): " + bytesToHex(key1.keyMaterial())); // Accessor returns ByteString
        System.out.println("  Hashed Material (Hex): " + bytesToHex(key1.hashedKeyMaterial())); // Accessor returns ByteString
        ...
        System.out.println("  Length of random part (Base32): " + (key2.keyString().length() - KEY_PREFIX.length()));
        ...
        // Store the immutable ByteString directly or convert back to byte[] if needed ONLY at the boundary
        ...
        // System.out.println("Hash bytes for DB: " + bytesToHex(hashBytesForDb)); // Using helper overload
    }
