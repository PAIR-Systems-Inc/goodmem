 ...
 *
 * <p>This service handles operations like retrieving user details and system initialization. It
 * connects to the PostgreSQL database using the provided configuration and implements the gRPC
 * service interface defined in the protocol buffer.
 */
public class UserServiceImpl extends UserServiceImplBase {
   ...
   *
   * <p>Lookup behavior: 1. If neither user_id nor email is set: Returns the current user based on
   * API key authentication 2. If user_id is set: Looks up user by ID 3. If email is set (and
   * user_id not set): Looks up user by email 4. If both are set: Looks up by user_id and ignores
   * email (with a warning)
   *
   * <p>All API calls (except InitializeSystem) require authentication. Possible error codes: -
   * NOT_FOUND: No matching user exists - UNAUTHENTICATED: The request has no valid authentication -
   * PERMISSION_DENIED: The authenticated user is not authorized to view the requested user's
   * information
   */
  @Override
  public void getUser(
      GetUserRequest request, StreamObserver<goodmem.v1.UserOuterClass.User> responseObserver) {
    ...
    boolean hasAnyPermission =
        authenticatedUser.hasPermission(Permission.DISPLAY_USER_ANY);
    boolean hasOwnPermission =
        authenticatedUser.hasPermission(Permission.DISPLAY_USER_OWN);
    ...
    boolean userIdProvided = request.hasUserId() && !request.getUserId().isEmpty();
    ...
    if (!hasAnyPermission) {
      ...
      if (userIdProvided) {
        ...
        if (userIdOr.isNotOk() || !userIdOr.getValue().equals(authenticatedUser.getId())) {
          Logger.error(
              "User with DISPLAY_USER_OWN permission attempted to access another user's data");
          responseObserver.onError(
              io.grpc.Status.PERMISSION_DENIED
                  .withDescription("Permission denied")
                  .asRuntimeException());
  ...
  public void initializeSystem(
      InitializeSystemRequest request, StreamObserver<InitializeSystemResponse> responseObserver) {
    ...
    try (Connection connection = config.dataSource().getConnection()) {
      ...
      InitializeSystemResponse.Builder responseBuilder =
          InitializeSystemResponse.newBuilder()
