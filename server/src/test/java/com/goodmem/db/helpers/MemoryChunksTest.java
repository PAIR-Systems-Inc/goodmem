...
public class MemoryChunksTest {
  ...
  void testLoadByMemoryId() {
    ...
    List<Integer> sequenceNumbers =
        result.getValue().stream().map(MemoryChunk::chunkSequenceNumber).toList();
  ...
  void testLoadByVectorStatus() {
    ...
    MemoryChunk generatedChunk =
        createTestChunk(
            testMemoryId, testUserId, 2, "Generated chunk", new float[] {0.1f, 0.2f, 0.3f});
    ...
    StatusOr<List<MemoryChunk>> pendingResult =
        MemoryChunks.loadByVectorStatus(connection, "PENDING", 10);
    StatusOr<List<MemoryChunk>> generatedResult =
        MemoryChunks.loadByVectorStatus(connection, "GENERATED", 10);
    StatusOr<List<MemoryChunk>> failedResult =
        MemoryChunks.loadByVectorStatus(connection, "FAILED", 10);
  ...
  void testUpdateVectorStatus() {
    ...
    StatusOr<Integer> updateResult =
        MemoryChunks.updateVectorStatus(connection, chunk.chunkId(), "GENERATED", testUserId);
  ...
  void testVectorSearch() {
    ...
    StatusOr<List<MemoryChunk>> searchResult =
        MemoryChunks.vectorSearch(connection, queryVector, testSpaceId, 3);
    ...
    List<String> chunkTexts = searchResult.getValue().stream().map(MemoryChunk::chunkText).toList();
  ...
  private static MemoryChunk createTestChunk(
      UUID memoryId, UUID userId, int sequenceNumber, String text, float[] vector) {
