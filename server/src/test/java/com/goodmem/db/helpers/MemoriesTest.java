...
public class MemoriesTest {
  ...
  void testLoadAll_ReturnsAllMemories_WhenMultipleExist() {
    ...
    List<String> refs = result.getValue().stream().map(Memory::originalContentRef).toList();
  ...
  void testLoadByProcessingStatus_ReturnsMemories_WhenMatch() {
    ...
    StatusOr<List<Memory>> completedResult =
        Memories.loadByProcessingStatus(connection, "COMPLETED");
  ...
  void testUpdateProcessingStatus_UpdatesStatus_WhenMemoryExists() {
    ...
    StatusOr<Integer> result =
        Memories.updateProcessingStatus(connection, memory.memoryId(), "PROCESSING", testUserId);
