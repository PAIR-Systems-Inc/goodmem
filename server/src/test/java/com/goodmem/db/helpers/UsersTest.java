...
public class UsersTest {
  ...
  static void setUp() throws SQLException {
    ...
    postgresContext = PostgresTestHelper.setupPostgres("goodmem_users_test", UsersTest.class);
  ...
  void testLoadAll_ReturnsAllUsers_WhenMultipleExist() {
    ...
    List<String> emails = result.getValue().stream().map(User::email).toList();
  ...
  private User createTestUser(String username, String email) {
    UUID userId = UUID.randomUUID();
    Instant now = Instant.now();
    return new User(userId, username, email, "Display Name for " + username, now, now);
  }
}