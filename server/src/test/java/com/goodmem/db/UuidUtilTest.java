...
public class UuidUtilTest {
  ...
  void testInvalidUuidStringConversion() {
    ...
    assertEquals(
        "INVALID_ARGUMENT: Invalid UUID string: not-a-uuid", result.getStatus().toString());
  }
  ...
  void testInvalidUuidBytesConversion() {
    ...
    assertEquals(
        "INVALID_ARGUMENT: UUID bytes must be 16 bytes long", shortResult.getStatus().toString());
  }
}
