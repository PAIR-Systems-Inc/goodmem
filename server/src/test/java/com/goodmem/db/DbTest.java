...
public class DbTest {
     ...
     * 
     * The pgvector/pgvector-postgresql Docker image is a community-maintained image
     * that includes the pgvector extension pre-installed.
     */
    @Container
    ...
    void testVectorOperations() throws SQLException {
        ...
        try (var stmt = connection.createStatement()) {
            ...
            rs = stmt.executeQuery("SELECT '[3.0, 4.0, 5.0]'::vector <-> embedding AS distance FROM vector_test");
            assertTrue(rs.next());
     ...
     * 
     * This test verifies that we can use the vector type and operations
     * without testing our actual database layer code.
     */
    @Test
